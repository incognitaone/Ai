...
def load_images():
    residents_json = requests.get(f"http://{config.get("WEB")}/residents/json")
    residents = residents_json.json()
    for resident in residents:
        photo_request = requests.get(f"http://{config.get("WEB")}/resident/photo/{resident['id']}")
        if not os.path.exists("imgs"):
            os.makedirs("imgs")
        path = os.path.abspath(os.path.join("imgs", f"{resident['id']}.jpg"))
        with open(path, 'wb') as f:
            f.write(photo_request.content)
        image = face_recognition.load_image_file(path)
        face_encoding = face_recognition.face_encodings(image)[0]
        known_face_encodings.append(face_encoding)
        known_face_names.append(f"{resident['id']}")
